  ...
  return (
    <div className="min-h-screen">
      <div className="@container container mx-auto max-w-5xl px-6 py-4 lg:py-8">
        ...
        <div className="mx-auto mt-8 grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3">
          {filteredDoc.map(([key, item]) => {
            const Icon = item.icon;
            return (
              <Link key={key} href={item.href} className="block">
                <Card className="border-[oklch(1 0 0/10%)] mx-auto w-full border-[#2e2c23]">
                  <CardHeader>
                    <div>
                      <Icon
                    ...
                    <CardDescription className="h-20 overflow-hidden text-ellipsis text-white">
                      {item.description}
                    </CardDescription>
                  </CardHeader>
                </Card>
